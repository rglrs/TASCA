...
const LandingPage = () => {
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center text-center px-4 md:px-8">
      <motion.section
        initial={{ opacity: 0, y: -50 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="mt-8 max-w-6xl flex flex-col md:flex-row items-center text-center md:text-left gap-6 md:gap-12"
      >
        {/* Bagian Teks */}
        <div className="md:w-1/2 order-2 md:order-1">
          <motion.h2
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="text-2xl md:text-4xl font-bold text-blue-600 leading-tight"
          >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className="mt-6 bg-blue-600 text-white px-6 py-3 rounded-xl shadow-md text-base md:text-lg hover:bg-blue-700 transition"
          >
            Coba Sekarang
          </motion.button>
        </div>
        ...
        >
          <img src={heroImage} alt="Hero TASCA" className="w-full max-w-sm md:max-w-md rounded-xl" />
        </motion.div>
      </motion.section>

      {/* Penjelasan Pomodoro */}
      <div className="mt-16 max-w-6xl w-full flex flex-col md:flex-row items-center text-center md:text-left gap-6 md:gap-12">
        {/* Gambar Mockup */}
        <motion.div
          initial={{ opacity: 0, x: -50 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        >
          <img src={mockupImage} alt="Mockup TASCA" className="w-full max-w-sm md:max-w-md rounded-xl" />
        </motion.div>
